...
def load_dataset(name):
    ...
    if name == "elec2":
        ...
        df['timestamp'] = pd.date_range(start='1996-5-7', end='1998-12-6 23:30:00', freq='30min', inclusive='both')
    ...
    if name == "changepoint":
        df = pd.read_csv("/data2/wujunxi/PID/tests/datasets/changepoint.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "distribution_drift":
        df = pd.read_csv("/data2/wujunxi/PID/tests/datasets/distribution_drift.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "heavy-tailed":
        df = pd.read_csv("/data2/wujunxi/PID/tests/datasets/heavy-tailed.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "variance_changepoint":
        df = pd.read_csv("/data2/wujunxi/PID/tests/datasets/variance_changepoint.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    if name == "extreme_drift":
        df = pd.read_csv("/data2/wujunxi/PID/tests/datasets/extreme_drift.csv")
        df.rename({'timestamp': 'timestamp', 'y': 'y'}, axis='columns', inplace=True)
        # df = df.drop("Unnamed: 0", axis='columns')
        data = df.melt(id_vars=['timestamp'], value_name='target')
        data.rename({'variable': 'item_id'}, axis='columns', inplace=True)
    ...
    data = data.infer_objects(copy=False)  # add before data = data.interpolate(), address FutureWarning
